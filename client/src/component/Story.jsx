  ...
  return (
    <div className='w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl overflow-x-auto no-scrollbar px-4'>
        <div className='flex gap-4 pb-5'>
          {/* Add Story Card */}
          <div className='rounded-lg shadow-sm min-w-30 max-w-50 max-h-40 aspect-[3/4] cursor-pointer hover:shadow-lg hover:scale-102 hover:translate-y-0.5 active:scale-95 transition-all duration-200 ease-in-out border-2 border-dashed border-indigo-300 bg-gradient-to-b from-indigo-50 to-white'>
          ...
          {storyDatas.map((storyData, index)=>(
            <div key={index} className='rounded-lg shadow-sm min-w-30 max-w-30 max-h-40 aspect-[3/4] cursor-pointer hover:scale-102 transition-all duration-200 ease-in-out hover:translate-y-0.5 active:scale-95 hover:bg-cyan-800 relative bg-cyan-500'>
              <img src={storyData.user.profile_picture} alt="" className='w-9 h-9 z-10 absolute left-1.5 top-1.5 rounded-full border-white border-3'/>
              {storyData.content ==="" ?
